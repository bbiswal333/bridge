describe("The bridgeConfigService", function () {
    ...
    it("should read the config from the local storage and send it to the backend", function(){
        $httpBackend.expectPOST('https://ifd.wdf.sap.corp/sap/bc/bridge/SETUSERCONFIG?instance=' + bridgeInstance.getCurrentInstance() + '&origin=' + encodeURIComponent($window.location.origin),
    ...
    it("should load a team view from backend", function() {
        $httpBackend.expectGET('http://ifd.wdf.sap.corp/sap/bc/bridge/GET_VIEW?view=viewId&owner=owner&instance=' + bridgeInstance.getCurrentInstance() + '&origin=' + encodeURIComponent($window.location.origin)).respond("200", {apps: []});
    ...
    it("should save own views via separate calls", function() {
        ...
        $httpBackend.expectPOST('https://ifd.wdf.sap.corp/sap/bc/bridge/SET_VIEW?view=testGuid&viewName=view name&instance=server&origin=' + encodeURIComponent($window.location.origin),
        function validate(data){
            var oData = angular.fromJson(data);
