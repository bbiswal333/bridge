	...
	this.findMatches = function(query, resultArray) {
		...
		promises = searchProviders.map(function(provider) {
			if (bridgeSettings.searchProvider[provider.getSourceInfo().name].selected) {
				var result = { info: provider.getSourceInfo(), results: [], callbackFn: provider.getCallbackFn(), resultInfo : { } };
				resultArray.push(result);
				var promise = provider.findMatches(query, result.results, result.resultInfo);
				if (promise) {
					return promise;
				}
			}
