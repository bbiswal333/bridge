describe("bridge map service", function() {
	...
	beforeEach(function() {
		...
		inject(["bridge.service.maps.routing", "$httpBackend", "$interval", function(_mapService, $httpBackend, $interval) {
			...
			httpBackend.whenGET(new RegExp('https:\\/\\/route.nlp.nokia.com\\/routing\\/7.2\\/calculateroute.json\\?waypoint0=link!!\\ServerDown&waypoint1=geo!49.41349,8.70807&alternatives=2&routeattributes=none,routeId&language=en-US&mode=fastest;car;traffic:disabled.*')).respond(500, '');
			...
			httpBackend.whenGET(new RegExp('https:\\/\\/route.nlp.nokia.com\\/routing\\/7.2\\/calculateroute.json\\?waypoint0=link!!\\+1599025847254060527&waypoint1=geo!49.41349,8.70807&alternatives=2&routeattributes=none,routeId&language=en-US&mode=fastest;car;traffic:disabled.*')).respond(ROUTES_RESPONSE);
			httpBackend.whenGET(new RegExp('https:\\/\\/route.nlp.nokia.com\\/routing\\/7.2\\/calculateroute.json\\?waypoint0=link!!\\NotAvailableLink&waypoint1=geo!49.41349,8.70807&alternatives=2&routeattributes=none,routeId&language=en-US&mode=fastest;car;traffic:disabled.*')).respond({"type":"ApplicationError","subtype":"LinkIdNotFound","details":"LinkId(s) specified in request are not available","additionalData":[{"key":"LinkId[0]","value":"-49411249870807"}],"metaInfo":{"timestamp":"2015-01-07T14:34:40Z","mapVersion":"8.30.56.155","moduleVersion":"7.2.52.0_CD-1094_1","interfaceVersion":"2.6.6"}});
			httpBackend.whenGET(new RegExp('https:\\/\\/route.nlp.nokia.com\\/routing\\/7.2\\/calculateroute.json\\?waypoint0=geo!49.2931652,8.6422384&waypoint1=link!!\\+1599025847254060527&waypoint2=link!!\\+1599025847254060871&waypoint3=link!!\\+1599025842908758185&waypoint4=link!!\\+1599377686612869120&waypoint5=link!!\\+1599377695219581129&waypoint6=link!!-1599377695219581103&waypoint7=link!!\\+1599377695219581111&alternatives=0&routeattributes=none,routeId&language=en-US&mode=fastest;car;traffic:disabled.*')).respond(REBUILD_ROUTE_ID);
	...
	it("should calculate routes between link and point (lat/lon)", function() {
		mapService.calculateAlternativeRoutesBetween({linkId: "+1599025847254060527"}, {position: {"latitude":49.41349,"longitude":8.70807}}, function(results) {
			expect(results.error).toBeFalsy();
			expect(results.routes.length).toEqual(3);
		});
		httpBackend.flush();
	...
	it("should fail to calculate routes between unknown points", function() {
		mapService.calculateAlternativeRoutesBetween({linkId: "NotAvailableLink"}, {position: {"latitude":49.41349,"longitude":8.70807}}, function(results) {
			expect(results.error).toBeTruthy();
	...
	it("should fail to calculate routes if the server is down", function() {
		mapService.calculateAlternativeRoutesBetween({linkId: "ServerDown"}, {position: {"latitude":49.41349,"longitude":8.70807}}, function(results) {
			expect(results.error).toBeTruthy();
			expect(results.routes.length).toEqual(0);
		});
		httpBackend.flush();
	...
	it("should rebuild a route from a number of given waypoints (mixed coordinates and links)", function() {
		mapService.rebuildRouteFromWaypoints([{position: {latitude: 49.2931652, longitude: 8.6422384}}, {linkId: '+1599025847254060527'}, {linkId : '+1599025847254060871'}, {linkId : '+1599025842908758185'}, {linkId : '+1599377686612869120'}, {linkId : '+1599377695219581129'}, {linkId : '-1599377695219581103'}, {linkId : '+1599377695219581111'}], function(result) {
	...
	it("should take care that routes update their data each 2.5 minutes", function() {
		...
		TEST_RESPONSE.response.route[0].routeId = "AHsACAAAAB4AAAA4AAAAmwAAAJYAAAB42mOYxsDAxMQABCxWqm0rpvAmM0BB50MDsfe89jYM//9DBD7sZ0ACXEC8WjU4i4mh0tGwratYGa6xR9FIzIHB3hKPxvQlWvqMQIvhgtkTHwX7AOkjAUwcDoZAxzAAABpKGWw6gHSb";
