angular.module('app.correctionWorkbench').service('app.correctionWorkbench.workbenchData', ['$http', '$q', '$interval', '$location', 'notifier', function ($http, $q, $interval, $location, notifier) {
    ...
    this.parseBackendData = function(backendData, source) {
	...
	this.loadWorkbenchData = function () {
        ...
        $http.post('https://css.wdf.sap.corp/sap(bD1lbiZjPTAwMQ==)/bc/bsp/spn/jcwb_srv/find_correction_requests?sap-language=EN',
            ...
            if (angular.isArray(payloadData.correctionRequests.correctionRequest)) {
                angular.forEach(payloadData.correctionRequests.correctionRequest, function (correctionRequest) {
                    that.parseBackendData(correctionRequest, 'onMyName');
                });
            } else if (angular.isObject(payloadData.correctionRequests)) {
                that.parseBackendData(payloadData.correctionRequests.correctionRequest, 'onMyName');
            }
        ...
        $http.post('https://css.wdf.sap.corp/sap(bD1lbiZjPTAwMQ==)/bc/bsp/spn/jcwb_srv/find_correction_requests?sap-language=EN',
            ...
            if (angular.isArray(payloadData.correctionRequests.correctionRequest)) {
                angular.forEach(payloadData.correctionRequests.correctionRequest, function (correctionRequest) {
                    that.parseBackendData(correctionRequest, 'plannedForTesting');
                });
            } else if (angular.isObject(payloadData.correctionRequests)) {
                that.parseBackendData(payloadData.correctionRequests.correctionRequest, 'plannedForTesting');
            }
