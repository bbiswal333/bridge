angular.module('app.rooms').service('ifpservice', [
    ...
    function($http, dataService, calUtils) {
        ...
        function extractDates(reservation) {
			...
			reservation.VALIDFROMDATE = new Date(Date.UTC(parseInt(m[1]),parseInt(m[2]) - 1,parseInt(m[3]),parseInt(m[4]),parseInt(m[5])));
			m = reservation.VALIDTO.toString().match(datePattern);
			reservation.VALIDTODATE = new Date(Date.UTC(parseInt(m[1]),parseInt(m[2]) - 1,parseInt(m[3]),parseInt(m[4]),parseInt(m[5])));
		}
