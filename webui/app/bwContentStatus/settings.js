angular.module('app.bwContentStatus').appContentStatusSettings =
	...
	if ($scope.values.contents.length == 0 ) $scope.values.contents = dataService.data.configContents;
	if ($scope.values.contents.length != dataService.data.configContents.length ) {
		...
		for (var i = 0; i < $scope.values.contents.length; i++) {
			for (var n = 0; n < tmp.length; n++) {
				if ($scope.values.contents[i].CONTID == tmp[n].CONTID && tmp[n].active) $scope.values.contents[i].active = true;
			};
		};
	};
